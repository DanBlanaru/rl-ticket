...
if __name__ == '__main__':
    ...
    model = ContinuousDeterministicModel(input_dims=state_shape[0], action_dims=action_dims).to(device)
    prune.ln_structured(model.policy_fc1, name="weight", amount=0.2, n=2, dim=0)

    # model2 = ContinuousDeterministicModel(input_dims = state_shape[0], action_dims = action_dims).to(device)
    # prune.ln_structured(model2.policy_fc1, name = "weight", amount = 0.2, n=2, dim=0)

    # state_dict = model.state_dict()
    # model2.load_state_dict(state_dict)
    # print(model2.policy_fc1.weight_orig,"\n\n", model.policy_fc1.weight_orig)

    # for p1,p2 in zip(model.named_parameters(),model2.named_parameters()):
    #     print(p1[0],(p1[1].data == p2[1].data).all())
    # quit()
    alg = DDPG(
        model=model,
